...
struct sMenuItem GraphicMenuItems[] = {{"Заставка", EmblemFunc, NULL},
							           {"Примитивы", ElementsFunc, NULL},
...
struct sMenuItem LEDsMenuItems[] = {{"Включить", LightsOnFunc, NULL},
									{"Тест Пада", PADINIT, NULL},
                                    {"Возврат", ReturnFunc, NULL}};
struct sMenu LEDsMenu = {"Светодиоды", LEDsMenuItems, countof(LEDsMenuItems)};

struct sMenuItem IFMenuItems[] = {{"USB", LightsOnFunc, NULL},
								  {"UART", InitUART, NULL},
								  {"CAN", LightsOnFunc, NULL},
								  {"TimerPWM", InitPWM, NULL},
                                  {"Возврат", ReturnFunc, NULL}};
struct sMenu IFMenu = {"Интерфейсы", IFMenuItems, countof(IFMenuItems)};

struct sMenuItem AnMenuItems[] =  {{"ADC", ADCTEST, NULL},
				//{{"ADC", InitADC, NULL},
								  {"DAC", InitDAC, NULL},
								  {"Компаратор", COMPINIT, NULL},
								  {"Генераторы", InitHSILSI, NULL},
                                  {"Возврат", ReturnFunc, NULL}};
struct sMenu AnMenu = {"Аналоговые блоки", AnMenuItems, countof(AnMenuItems)};

struct sMenuItem MCUMenuItems[] = 
								{{"Дата и время", ClockFunc, NULL},
							   {"Аналоговые блоки", IdleFunc, &AnMenu},
							   {"Интерфейсы",IdleFunc,&IFMenu},
                               {"Возврат", ReturnFunc, NULL}};
struct sMenu MCUMenu = {"Микроконтрллер", MCUMenuItems, countof(MCUMenuItems)};
...
struct sMenuItem MainMenuItems[] = {
  ...
  {"Микроконтроллер", IdleFunc, &MCUMenu},
